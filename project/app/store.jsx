// Shared state, theme, copy, products data — all Russian.
...
// ───────────────────────────────────────────────
// Products — 20 items
// Image is a CSS gradient + text (since we can't fetch real product photos).
// Each carries dominant color tuple for the placeholder.
// ───────────────────────────────────────────────
const PRODUCTS = [
...
const pctOff = (price, old) => Math.round((1 - price / old) * 100);

// ───────────────────────────────────────────────
// Cart store via React context
// ───────────────────────────────────────────────
...
function CartProvider({ children }) {
  const [items, setItems] = React.useState({}); // {productId: qty}
...
// Theme context
// ───────────────────────────────────────────────
const ThemeContext = React.createContext(THEMES.magnit);
const useTheme = () => React.useContext(ThemeContext);

// ───────────────────────────────────────────────
// Router context — single screen + params
// ───────────────────────────────────────────────
const RouterContext = React.createContext(null);
const useRouter = () => React.useContext(RouterContext);

function RouterProvider({ initial = { screen: 'home' }, children }) {
...
Object.assign(window, {
  THEMES, CATEGORIES, PRODUCTS, BANNERS,
  fmtRub, pctOff,
