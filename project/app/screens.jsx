// Screen renderers — Home, Catalog (PLP), PDP, Cart, Checkout, OrderDone.
// Each takes a `device` prop ('mobile' | 'desktop') so they can shift layout.
...
function HomeScreen({ device }) {
  ...
  return (
    <div style={{ background: t.bg, color: t.ink, minHeight: '100%', paddingBottom: isDesk ? 60 : 16 }}>
      ...
      {isDesk && (
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 24, alignItems: 'center' }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, opacity: 0.95 }}>{Icon.truck({ width: 14, height: 14 })} Доставка по РФ от 1 дня</span>
      ...
      {/* Category chips */}
      <div style={{
        display: 'flex', gap: 10, padding: isDesk ? '24px 40px 8px' : '12px 16px',
        ...
        <div style={{
          ...
          {['Nivea', 'Pantene', 'Loreal', 'Garnier', 'Dove', 'Maybelline', 'Schwarzkopf', 'Vichy'].map((b) => (
            <div key={b} style={{
              ...
              fontWeight: 800, fontSize: isDesk ? 14 : 12, textAlign: 'center',
              letterSpacing: '0.02em',
            }}>{b}</div>
          ))}
        </div>
...
function Section({ title, sub, children, device, onSeeAll }) {
  const t = useTheme();
  const isDesk = device === 'desktop';
  return (
    <div style={{ marginTop: isDesk ? 36 : 20 }}>
      <div style={{
        padding: isDesk ? '0 40px 14px' : '0 16px 10px',
        ...
        {onSeeAll && (
          <button onClick={onSeeAll} style={{
            ...
            color: t.primary, fontWeight: 800, fontSize: isDesk ? 14 : 13,
            whiteSpace: 'nowrap',
          }}>смотреть все →</button>
        )}
      </div>
...
function Carousel({ children, device }) {
  const isDesk = device === 'desktop';
  return (
    <div style={{
...
function CatalogScreen({ device }) {
  ...
  return (
    <div style={{ background: t.bg, color: t.ink, minHeight: '100%' }}>
      {/* Header band */}
      <div style={{
        background: t.surfaceAlt, padding: isDesk ? '24px 40px 20px' : '12px 16px 14px',
      }}>
      ...
      <div style={{
        ...
        <Chip active={filter.onlySale} onClick={() => setFilter((f) => ({ ...f, onlySale: !f.onlySale }))}>
          🔥 Со скидкой
        </Chip>
        <Chip active={filter.hitsOnly} onClick={() => setFilter((f) => ({ ...f, hitsOnly: !f.hitsOnly }))}>
          ⭐ Хиты
        </Chip>
  ...
  return (
    <div style={{ background: t.bg, color: t.ink, minHeight: '100%' }}>
      <div style={{
        ...
        <div>
          <div style={{
            background: t.surface, padding: isDesk ? 32 : 0, borderRadius: isDesk ? 16 : 0,
            position: 'relative',
          }}>
          ...
          {/* thumbnail strip */}
          <div style={{
            display: 'flex', gap: 8, padding: isDesk ? '12px 0 0' : '12px 16px 0',
            overflowX: 'auto', scrollbarWidth: 'none',
          }}>
            {[0, 1, 2, 3].map((i) => (
              <div key={i} style={{
        ...
        <div style={{ padding: isDesk ? 0 : '16px 16px 0' }}>
          ...
          <h1 style={{ fontSize: isDesk ? 30 : 22, fontWeight: 900, margin: '6px 0 8px',
            letterSpacing: '-0.02em', lineHeight: 1.15, textWrap: 'pretty' }}>{p.name}</h1>
          ...
          <div style={{
            ...
            <div>
              <div style={{
                fontSize: isDesk ? 38 : 30, fontWeight: 900, color: t.primary,
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 16, alignItems: 'stretch' }}>
            ...
            <div style={{
              ...
              background: t.surface, borderRadius: 12, boxShadow: `inset 0 0 0 1.5px ${t.border}`,
              padding: 4, gap: 4,
            }}>
          ...
          <div style={{ padding: '16px 0', fontSize: 14, lineHeight: 1.6, color: t.ink }}>
            {tab === 'about' && (
              <p style={{ margin: 0, textWrap: 'pretty', maxWidth: 560 }}>
...
function CartScreen({ device }) {
  ...
  const promo = 0;
  const total = cart.subtotal + delivery - promo;
  ...
  return (
    <div style={{ background: t.bg, color: t.ink, minHeight: '100%' }}>
      <div style={{
        ...
        <div>
          <h1 style={{
            fontSize: isDesk ? 32 : 22, fontWeight: 900, letterSpacing: '-0.02em',
            margin: 0, padding: isDesk ? 0 : '14px 16px 8px',
          }}>Корзина · {cart.count} {cart.count === 1 ? 'товар' : (cart.count < 5 ? 'товара' : 'товаров')}</h1>

          <div style={{ display: 'flex', flexDirection: 'column', gap: 10,
            padding: isDesk ? '20px 0 0' : '0 16px' }}>
            {cart.list.map((p) => (
              <CartLine key={p.id} p={p} />
            ))}
          </div>

          {/* Promo code field */}
          <div style={{
            display: 'flex', gap: 8, padding: isDesk ? '20px 0 0' : '16px',
          }}>
        ...
        <div style={{ padding: isDesk ? 0 : '8px 16px 16px', position: isDesk ? 'sticky' : 'static', top: 20, alignSelf: 'flex-start' }}>
          <div style={{
            ...
            <div style={{ borderTop: `1.5px dashed ${t.border}`, margin: '12px 0 12px' }} />
            ...
            <div style={{ display: 'flex', gap: 6, justifyContent: 'center', marginTop: 12, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <PayBadge>МИР</PayBadge>
              <PayBadge>СБП</PayBadge>
              <PayBadge>Долями</PayBadge>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8, flexShrink: 0 }}>
        ...
        <div style={{
          display: 'flex', alignItems: 'center',
          background: t.surfaceAlt, borderRadius: 999, padding: 3,
        }}>
          <button onClick={() => cart.setQty(p.id, p.qty - 1)} style={{
            ...qtyBtn(t), width: 28, height: 28, color: p.qty === 1 ? t.muted : t.ink,
          }}>{p.qty === 1 ? Icon.trash() : Icon.minus()}</button>
    ...
    <div style={{ background: t.bg, color: t.ink, minHeight: '100%' }}>
      <div style={{
        ...
        <div>
          ...
          {/* Steps */}
          <Stepper step={step} steps={['Контакты', 'Доставка', 'Оплата']} />
          ...
          <Block title="2. Доставка">
            <RadioRow checked={delivery === 'courier'} onClick={() => setDelivery('courier')}
              t="Курьер" s="Завтра до 22:00 · 199 ₽ (бесплатно от 1 500 ₽)" />
            <RadioRow checked={delivery === 'pickup'} onClick={() => setDelivery('pickup')}
              t="Самовывоз" s="Сегодня после 18:00 · бесплатно · 12 точек в Москве" />
            <RadioRow checked={delivery === 'post'} onClick={() => setDelivery('post')}
              t="Почта России / СДЭК" s="3–7 дней · от 199 ₽" />
            {delivery !== 'pickup' && (
          ...
          <Block title="3. Оплата">
            <RadioRow checked={pay === 'sbp'} onClick={() => setPay('sbp')}
              t="СБП" s="Через приложение банка · без комиссии" />
            <RadioRow checked={pay === 'card'} onClick={() => setPay('card')}
              t="Картой онлайн" s="Visa, Mastercard, МИР" />
            <RadioRow checked={pay === 'split'} onClick={() => setPay('split')}
              t="Долями" s="4 платежа без переплат · одобрение за 1 минуту" />
            <RadioRow checked={pay === 'cash'} onClick={() => setPay('cash')}
              t="При получении" s="Наличными или картой курьеру" />
          </Block>

          {!isDesk && (
            <Button block size="lg" onClick={place} style={{ marginTop: 16 }}>
              Оформить · {fmtRub(total)}
            </Button>
          )}
        </div>
        ...
        <div style={{ position: isDesk ? 'sticky' : 'static', top: 20, alignSelf: 'flex-start' }}>
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 12,
              ...
              {cart.list.length > 4 && (
                <div style={{ fontSize: 12, color: t.muted }}>и ещё {cart.list.length - 4}…</div>
              )}
            </div>
            <Row k="Товары" v={fmtRub(cart.subtotal)} />
            <Row k="Скидка" v={`−${fmtRub(cart.saved)}`} accent />
...
function Block({ title, children }) {
  ...
  return (
    <div style={{
      background: t.surface, borderRadius: 16, padding: 18, marginBottom: 12,
      boxShadow: `inset 0 0 0 1px ${t.border}`,
    }}>
...
// ORDER DONE
// ───────────────────────────────────────────────
function OrderDoneScreen({ device }) {
  const t = useTheme();
  const router = useRouter();
...
// ───────────────────────────────────────────────
// Desktop footer (used at the bottom of every desktop screen)
// ───────────────────────────────────────────────
function DesktopFooter() {
  ...
  return (
    <div style={{
      background: t.headerBg, color: t.headerInk,
      padding: '40px 40px 24px', marginTop: 40,
    }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr 1fr', gap: 40, marginBottom: 24 }}>
        ...
        {[
          ...
          <div key={col.h}>
            ...
            {col.l.map((x) => (
              <div key={x} style={{ fontSize: 13, opacity: 0.9, marginBottom: 6, cursor: 'pointer' }}>{x}</div>
            ))}
          </div>
        ))}
      </div>
...
Object.assign(window, {
  HomeScreen, CatalogScreen, PDPScreen, CartScreen, CheckoutScreen, OrderDoneScreen,
  DesktopFooter, Section, Carousel, Chip, CartLine, PayBadge, Stepper, Block, Field, RadioRow,
});
