...
export function DesignCanvas({ children, minScale, maxScale, style }) {
  ...
  React.useEffect(() => {
    let off = false;
    // Try to load saved state from localStorage (replaces file-based sidecar)
    try {
      const saved = JSON.parse(localStorage.getItem('dc-canvas-state') || 'null');
  ...
  React.useEffect(() => {
    ...
    const t = setTimeout(() => {
      try { localStorage.setItem('dc-canvas-state', JSON.stringify({ sections: state.sections })); } catch {}
    }, 250);
...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  React.useEffect(() => {
    ...
    let gsBase = 1, isGesturing = false;
